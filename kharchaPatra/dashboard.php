...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_income'])) {
        ...
        header("Location: dashboard.php?added=income");
        exit;
        ...
        header("Location: dashboard.php?added=expense");
        exit;
    } elseif (isset($_POST['add_saving'])) {
        $note = trim($_POST['saving_note']);
        $amount = floatval($_POST['saving_amount']);
        if ($note !== '' && $amount > 0) {
            $stmt = mysqli_prepare($conn, "INSERT INTO savings (user_id, amount, note, saving_date) VALUES (?, ?, ?, CURDATE())");
            mysqli_stmt_bind_param($stmt, "ids", $user_id, $amount, $note);
            mysqli_stmt_execute($stmt);
        }
        header("Location: dashboard.php?added=saving");
        exit;
    }
}
...
// expense categories for the quick-add dropdown
$catStmt = mysqli_prepare($conn, "SELECT id, name FROM categories WHERE user_id = ? AND type = 'expense' ORDER BY name");
mysqli_stmt_bind_param($catStmt, "i", $user_id);
mysqli_stmt_execute($catStmt);
$categories = mysqli_stmt_get_result($catStmt);

// ---- Recent transactions (last 6, income + expenses combined) ----
$recentStmt = mysqli_prepare($conn, "
    (SELECT 'income' AS type, source AS label, amount, income_date AS txn_date FROM income WHERE user_id = ?)
    UNION ALL
    (SELECT 'expense' AS type, c.name AS label, e.amount, e.expense_date AS txn_date
        ...
        <div class="page-body">
            ...
            <?php if (isset($_GET['added'])): ?>
                <div class="alert alert-success" style="max-width:400px;">
                    <?= $_GET['added'] === 'income' ? 'Income added.' : ($_GET['added'] === 'expense' ? 'Expense added.' : 'Saving added.') ?>
                </div>
            <?php endif; ?>
            ...
            <div class="quick-add-grid quick-add-grid-3">
                ...
                <div class="quick-add-card">
                    <form method="POST" action="dashboard.php">
                        <div class="form-group">
                            <label>Note</label>
                            <input type="text" name="saving_note" placeholder="e.g. Emergency fund" required>
                        </div>
                        <div class="form-group">
                            <label>Amount</label>
                            <input type="number" step="0.01" name="saving_amount" placeholder="0.00" required>
                        </div>
                        <button type="submit" name="add_saving" class="btn btn-saving btn-full">+ Add saving</button>
                    </form>
                </div>
            </div>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        <?php if (mysqli_num_rows($recent) === 0): ?>
                            <tr><td colspan="4">No transactions yet. Add your first income or expense above.</td></tr>
                        <?php endif; ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($r['type'] === 'income'): ?>
                                    <span class="tag tag-income">Income</span>
                                <?php else: ?>
                                    <span class="tag tag-expense">Expense</span>
                                <?php endif; ?>
                            </td>
                            <td><?= htmlspecialchars($r['label']) ?></td>
                            <td><?= $r['type'] === 'income' ? '+' : '-' ?> Rs. <?= number_format($r['amount'], 2) ?></td>
                        </tr>
                        <?php endwhile; ?>
                    </tbody>
                </table>
