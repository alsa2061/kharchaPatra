...
// ---- Balance trend + expenses over the period, same style as the dashboard charts ----
// Group by day if the range is 62 days or less, otherwise group by month (e.g. for a full year).
$daysInRange = (strtotime($endDate) - strtotime($startDate)) / 86400;
$groupByMonth = $daysInRange > 62;

$chartLabels  = [];
$chartBalance = [];
$chartExpenses = [];

function periodSum($conn, $user_id, $table, $dateCol, $from, $to) {
    $stmt = mysqli_prepare($conn, "SELECT COALESCE(SUM(amount),0) FROM $table WHERE user_id=? AND $dateCol BETWEEN ? AND ?");
    mysqli_stmt_bind_param($stmt, "iss", $user_id, $from, $to);
    mysqli_stmt_execute($stmt);
    return floatval(mysqli_fetch_row(mysqli_stmt_get_result($stmt))[0]);
}
...
if ($groupByMonth) {
    ...
    while ($cursor <= $end) {
        ...
        $inc = periodSum($conn, $user_id, 'income', 'income_date', $monthStart, $monthEnd);
        $exp = periodSum($conn, $user_id, 'expenses', 'expense_date', $monthStart, $monthEnd);
        $chartBalance[] = $inc - $exp;
        $chartExpenses[] = $exp;
    ...
    while ($cursor <= $end) {
        ...
        $inc = periodSum($conn, $user_id, 'income', 'income_date', $day, $day);
        $exp = periodSum($conn, $user_id, 'expenses', 'expense_date', $day, $day);
        $chartBalance[] = $inc - $exp;
        $chartExpenses[] = $exp;
    ...
    <div class="main-content">
        ...
        <div class="page-body">
            ...
            <form class="filter-bar" method="GET" action="reports.php">
                ...
                <a href="report_print.php?<?= htmlspecialchars($_SERVER['QUERY_STRING']) ?>" class="btn btn-outline" style="align-self:flex-end;" target="_blank">⬇ Download PDF</a>
            </form>
            ...
            <div class="chart-grid">
                ...
                <div class="chart-card">
                    <h3>Expenses</h3>
                    <canvas id="reportBarChart" height="180"></canvas>
                </div>
            </div>
            ...
            <div class="table-card" style="margin-bottom:20px;">
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        <?php mysqli_data_seek($incomeRows, 0); ?>
            ...
            <div class="table-card">
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        <?php mysqli_data_seek($expenseRows, 0); ?>
